...
ROOT.gSystem.Load('libPhysicsToolsCPVNano')
...
#handles['prunedgen']    = ('prunedGenParticles'       , Handle('vector<reco::GenParticle>'))
#handles['packedgen']    = ('packedGenParticles'       , Handle('vector<pat::PackedGenParticle>'))
handles['genInfo' ] = ('generator'      , Handle('GenEventInfoProduct'           ))
handles['beamspot'] = ('offlineBeamSpot', Handle('reco::BeamSpot '               ))
handles['muons']  = ('slimmedMuons'             , Handle('vector<pat::Muon>'))
...
handles['tracks']        = ('packedPFCandidates'       , Handle('vector<pat::PackedCandidate>'))
handles['lostTracks']    = ('lostTracks'               , Handle('vector<pat::PackedCandidate>'))
...
if doDispSTMuons:
  suffix = '_dsa_mini'
  if doDisplTracks:
    suffix = '_displAll_mini'


events = Events( ['root://t3dcachedb.psi.ch:1094/' + i for i in glob('/pnfs/psi.ch/cms/trivcat/store/user/mratti/BHNLsGen/V20_test/mass3.0_ctau184.256851021/step4_enrichedV2_nj*.root')])
#events = Events( ['root://t3dcachedb.psi.ch:1094/' + i for i in glob('/pnfs/psi.ch/cms/trivcat/store/user/mratti/BHNLsGen/V20_test/mass3.0_ctau184.256851021/step4_enrichedV2_nj5*.root')])
#events = Events( ['root://t3dcachedb.psi.ch:1094/' + i for i in glob('/pnfs/psi.ch/cms/trivcat/store/user/mratti/BHNLsGen/V25/mass3.0_ctau2000.0/step4_nj*.root')])

fout = ROOT.TFile('bhnl_recoeff_V20_test{}.root'.format(suffix), 'recreate')
...
for i, event in enumerate(events):
    ...
    all_reco_tracks += [itk for itk in event.lostTracks]
    ...
    #event.genp = [ip for ip in event.prunedgen] + [ip for ip in event.packedgen] 
      ...
      for imutk in reco_mutks:
        ...
        if abs(imutk.mass-3) < 0.5:  # and this is loose..
          ...
          if hasattr(mu,'muonBestTrack'):
            ...
            tofit.push_back(mu)
          if tk.hasTrackDetails():
            tofit.push_back(tk.bestTrack())
          else:
            continue
